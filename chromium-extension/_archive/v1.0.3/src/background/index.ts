...
function buildRuntimeLLMSelection(rawLLMConfig: any): RuntimeLLMSelection {
  ...
  const isLocalProvider = bridgeRouted || isOllamaProvider || isOpenAICompatLocal;
...
async function loadLLMs(options?: {
  ...
  const directProvidersEnabled =
    policyMode === "all_providers_bridge_governed";
  ...
  const llms: LLMs = {
    default: {
      ...
      apiKey: async () => {
        ...
        if (current.isDirectProvider && mode !== "all_providers_bridge_governed") {
          throw new Error("provider_not_allowed");
        }
...
async function createChatAgentInstance(
  ...
  const nextAgent = new ChatAgent({ llms, agents }, chatId, undefined, socaTools);
