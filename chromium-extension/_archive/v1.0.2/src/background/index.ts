...
async function loadLLMs(): Promise<LLMs> {
  ...
  const allowDirectProviders = await getAllowDirectProviders();
  const lane = normalizeLane(
    (await chrome.storage.local.get([SOCA_LANE_STORAGE_KEY]))[
      SOCA_LANE_STORAGE_KEY
    ]
  );
  const directProvidersEnabled =
    allowDirectProviders && lane === "OB_ONLINE_PULSE";

  const baseURLRaw = String(llmConfig?.options?.baseURL || "").trim();
  let baseURLHost = "";
  if (baseURLRaw) {
  ...
  const openaiCompatLocal =
    ...
    baseURLHost &&
    isLocalHost(baseURLHost);
  const isLocalProvider =
    providerId === "soca-bridge" ||
    providerId === "ollama" ||
    openaiCompatLocal;

  // Hard fail-closed unless direct providers explicitly enabled in OB_ONLINE_PULSE.
  if (!isLocalProvider && !directProvidersEnabled) {
    printLog(
      `Direct provider '${providerId}' is disabled. Enable direct providers or use 'soca-bridge' (recommended) or local 'ollama'.`,
      "error"
    );
  ...
  const llms: LLMs = {
    default: {
      ...
      apiKey: async () => {
        ...
        const currentBaseURL = String(stored?.options?.baseURL || "").trim();
        const compatLocal =
          provider === "openai-compatible" &&
          currentBaseURL &&
          isLocalHost(new URL(currentBaseURL).hostname);
        ...
        const directAllowed =
          allowDirect && lane === "OB_ONLINE_PULSE" && !compatLocal;
        if (!compatLocal && !directAllowed) {
          throw new Error("provider_not_allowed");
        }
      ...
      config: {
        baseURL: async () => {
          ...
          if (provider === "openai-compatible") {
            ...
            if (!allowDirect || lane !== "OB_ONLINE_PULSE") {
              throw new Error("provider_not_allowed");
            }
            if (u.protocol !== "https:") {
          ...
          if (new URL(currentBaseURL).protocol !== "https:") {
            throw new Error("direct_baseURL_requires_https");
          }
          if (isLocalHost(new URL(currentBaseURL).hostname)) {
            throw new Error("direct_baseURL_local_host");
          }
          const allowDirect = await getAllowDirectProviders();
          const lane = normalizeLane(
