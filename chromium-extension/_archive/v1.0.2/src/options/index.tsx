...
const OptionsPage = () => {
  ...
  const [allowDirectProviders, setAllowDirectProviders] = useState(false);
  ...
  useEffect(() => {
    const loadDirectProviderGate = async () => {
      try {
        const result = await chrome.storage.local.get([
          SOCA_DIRECT_PROVIDER_GATE_KEY
        ]);
        setAllowDirectProviders(Boolean(result[SOCA_DIRECT_PROVIDER_GATE_KEY]));
      } catch (error) {
        console.warn("Failed to load direct provider gate:", error);
      }
    };
    loadDirectProviderGate();
  }, []);
  ...
  const handleDirectProvidersToggle = async (next: boolean) => {
    try {
      if (next) {
      ...
      setAllowDirectProviders(next);
      await chrome.storage.local.set({
        [SOCA_DIRECT_PROVIDER_GATE_KEY]: next
      });
      await runtimeSendMessage({ type: "SOCA_REFRESH_DNR" });
    } catch (error) {
      console.error("Failed to update direct provider gate:", error);
      message.error("Failed to update direct provider setting.");
    }
  };
  ...
  const openaiCompatLocal =
    providerId === "openai-compatible" && isLocalBaseURL(baseURL);
  const isDirectProvider =
    DIRECT_PROVIDER_IDS.has(providerId) ||
    (providerId === "openai-compatible" && !openaiCompatLocal);
  const directProviderBlocked = isDirectProvider && !allowDirectProviders;
  ...
  const apiKeyPlaceholder =
    providerId === "soca-bridge" ? "Paste bridge token" : "Paste API key";
          ...
          >
            ...
            <div className="mb-4 rounded-lg border border-theme-input bg-theme-input p-3">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-theme-primary">
                  Allow direct providers (network egress)
                </span>
                <Switch
                  checked={allowDirectProviders}
                  onChange={handleDirectProvidersToggle}
                />
              </div>
              <div
                ...
                style={{ opacity: 0.7 }}
              >
                When enabled, OpenBrowser can call direct cloud LLM endpoints.
                This requires optional host permissions and may send data over
                the network.
              </div>
            </div>
            ...
            {directProviderBlocked && (
              <Alert
                ...
                description="Enable 'Allow direct providers' above to use this provider."
              />
            )}
                ...
                ({ getFieldValue }) => ({
                  validator: async (_rule, value) => {
                    ...
                    const isDirect =
                      ...
                      (provider === "openai-compatible" && !isCompatLocal);
                    if (isDirect && !allowDirectProviders) {
                      throw new Error(
                        "Direct providers disabled. Enable above to continue."
                      );
                    }
